import { alert, confirm } from '../data/alert';
...
const WOAlertStory = {
  // 좌측 메뉴 depth
  title: 'WooriMobile/Modal/WOAlert',
  // 스토리북 컴포넌트 뷰 레이아웃
  parameters: {
    layout: 'centered',
  },
...
export const Alert = () => {
  ...
  return (
    <>
      <button type='button' onClick={() => handleAlert('조회 오류', '조회 가능한 계좌번호가 없습니다.')}>
        열기
      </button>
    </>
  );
  ...
  return (
    <>
      <button
        type='button'
        onClick={() =>
          handleAlert('보고서 조회 관리(일괄승인)', '일괄 초기화하시겠습니까?\n기관 결재 내역까지 모두 초기화됩니다.\n(아니시라면 "취소"를 누르세요)')
        }>
        열기
      </button>
    </>
