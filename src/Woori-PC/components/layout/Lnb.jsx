          ...
          {menuList.map((menu, index) => {
            ...
            if (index === menuList.length - 1) {
              ...
              return (
                <WOMenuIconButton
                  key={menu.id}
                  imageFile={`/assets/woori-pc//menu/${menu.imageName}`}
                  text={menu.title}
...
const Logo = styled(Link)`
  ...
  height: 36px;
  margin: 0 auto;
  background: url(/assets/woori-pc//menu/logo.svg) no-repeat center / cover;
`;
...
const SubMenuList = styled.ul`
  ...
  .content-button {
    ...
    padding: 10px 22px;
    width: 100%;
    background: url('/assets/woori-pc//lnb-right.svg') no-repeat 2px 14px/14px 16px;
  }
`;
